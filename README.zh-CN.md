...
不能信的反馈只是奉承——所以这个仓库给的是证据，不只是声明：

- **[通用 AI vs. 模拟同行评审 →](examples/)** —— 同一份有硬伤的草稿，对比通用聊天机器人如何给出空泛好评和 8/10、却漏掉致命缺陷，与本 skill 给出的、有学术标准支撑的批判。
- **[评审如何工作 →](METHODOLOGY.md)** —— 五个评审视角、按论文类型套用的真实报告标准（STROBE、CONSORT、PRISMA、COREQ、Toulmin、NIH），以及分数如何按稿件阶段校准。
- **[已知局限 →](LIMITATIONS.md)** —— 我们已知的失败案例，白纸黑字写出来。它辅助同行评审，而非取代。
...
git clone https://github.com/academicatstool-netizen/Cat_paper_review.git ~/.claude/skills/paper-review
```
